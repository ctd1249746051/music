...
function ajaxSearch() {
    ...
    $.ajax({
        ...
        success: function(jsonData){
            ...
            {
                if(jsonData.data.music_list.length === 0)   // 返回结果为零
                {
            ...
            for (var i = 0; i < jsonData.data.music_list.length; i++) {
                no ++;
                tempItem =  {
                    // id: jsonData.data.music_list[i].id,  // 音乐ID
                    name: jsonData.data.music_list[i].song_name.firstSourceText,  // 音乐名字
                    artist: jsonData.data.music_list[i].singer_name.firstSourceText, // 艺术家名字
                    album: '无',    // 专辑名字
                    source: jsonData.data.music_list[i].song_url,     // 音乐来源
                    // url_id: jsonData.data.music_list[i].url_id,  // 链接ID
                    // pic_id: jsonData.data.music_list[i].pic_id,  // 封面ID
                    // lyric_id: jsonData.data.music_list[i].lyric_id,  // 歌词ID
                    pic: null,    // 专辑图片
                    url: null   // mp3链接
                };
...
function ajaxUrl(music, callback)
{
    ...
    $.ajax({ 
        type: mkPlay.method,
        url: mkPlay.api,
        data: "url=" + music.source,
        dataType : "json",
        success: function(jsonData){
            // 调试信息输出
            // if(mkPlayer.debug) {
            //     console.debug("歌曲链接：" + jsonData.data.music.mp3);
            // }
            ...
            if(jsonData.data.music.mp3 === "") {
                music.url = "err";
            } else {
                music.url = jsonData.data.music.mp3;    // 记录结果 mp3
                music.lyrics = jsonData.data.music.lyrics.firstSourceText;    // 记录结果  歌词
                music.pic = jsonData.data.music.pic.firstSourceText;    // 记录结果  封面
            }
