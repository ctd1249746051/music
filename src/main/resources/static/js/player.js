...
//删除歌曲
function listDelete(no) {
    let temp = [];
    for (let i = 0; i < musicList[rem.dislist].item.length; i++) {
        if (i !== no)
            temp.push(musicList[rem.dislist].item[i])
    }

    musicList[rem.dislist].item = temp;
    playerSavedata('playing', musicList[1].item);
    playerSavedata('his', musicList[rem.dislist].item);
    // 初始化播放列表
    initList();
}
...
console.info('微信公众号：编程服务生活');
console.info('微信号：codeFL');
